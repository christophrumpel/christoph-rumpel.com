...
I was so nervous, that I didn't know how to move on anymore. We decided to remove that part of the talk and to show slides instead explaining the command. It was a good reminder of `everything that can fail, will fail`.

The following years, I only showed slides in my talks. There is so much less that can go wrong, and it makes you feel much safer while presenting.
...
I tried this approach a few times, but it didn't work. When you explain complex concepts, it gets confusing very fast. Think of all the files included in explaining Laravel's Routing. The viewer would see so many lines of code, which are not even relevant to what I was explaining. The `Router` class is over a thousand lines of code.

That's why I decided to only show specific code snippets in slides. This way I have much more control on what the current focus is, and I can guide the viewer better. 
