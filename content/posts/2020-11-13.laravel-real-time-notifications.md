...
The endpoint is now defined but returns a 419` error. This one is about the `csrf` token. We need it to verify this request. We don't have a form where we usually use it, but we can add it to our HTML head in a meta tag.

```html
<meta name="csrf-token" content="{{ csrf_token() }}">
```

Again we were able to solve a problem, and also we saw a new error. But trust me; we are getting close now :-)
...
Next to the broadcast routes, the BroadcastServiceProvider also activates the `routes/channels.php` file. Here is where we define who is allowed to access a private channel. There is an example, but we add our channel check now.
...
<div class="blognote"><strong>Note:</strong> If you use this branch, make sure to change the certificate paths to your ones in the WebSockets config file.</div>
