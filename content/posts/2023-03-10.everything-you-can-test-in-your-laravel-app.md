...
Notifications are great to inform your users about important events. Make sure to test them too:
...
In the example above, we are testing a notification that is sent to a user when a new product is created. We are using the `artisan` method to trigger the notification. This is a great way to test notifications that are triggered by a command.

Again, there is a `fake` method on notification facade that makes sure no actual notification is being sent.
