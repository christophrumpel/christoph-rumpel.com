<x-main-layout title="Privacy Policy Refactoring PHP" page="privacy-policy-rp">
    <main class="post-markdown-styles">
        <article>

            <h1>Business Privacy Policy - refactoring-php.com</h1>

            <p>This privacy policy sets out how Christoph Rumpel uses and protects any information that you give Christoph Rumpel when you visit <a href="https://refactoring-php.com/">refactoring-php.com</a>.
            </p>

            <p>Christoph Rumpel is committed to ensuring that your privacy is protected. Should we ask you to provide
                certain information by which you can be identified when using refactoring-php.com, then you can be
                assured that it will only be used in accordance with this privacy statement.</p>

            <p>Christoph Rumpel may change this policy from time to time by updating this page. You should check this
                page
                from time to time to ensure that you are happy with any changes. This policy is effective from
                20.01.2022.</p>

            <h2>What We Collect</h2>

            <p>We may collect the following information if you subscribe to the newsletter:</p>
            <ul>
                <li>Email address</li>
            </ul>

            <h2>What We Do with the Information We Gather</h2>

            <p>We do need this information in order to:</p>
            <ul>
                <li>Send news about Refactoring PHP</li>
                <li>To log you in</li>
                <li>If you purchase Refactoring PHP, we need to send you your invoice.</li>

            </ul>
            <p>Who the data is shared with:</p>
            <ul>
                <li>The application is deployed on servers from <a href="https://www.hetzner.com/legal/privacy-policy">Hetzner</a>
                    and <a href="https://all-inkl.com/datenschutz/">ALL-INKL.COM</a></li>
                <li>The email is sent through <a href="https://mailcoach.app/privacy">Mailcoach</a> and <a href="https://postmarkapp.com/eu-privacy">Postmark</a>.
                </li>
                <li>The payment process is done via <a href="https://paddle.com/privacy/">Paddle</a>.
                </li>
            </ul>
            <h2>How Long the Information Is Stored</h2>
            <p>If you are only subscribed to the newsletter, the information is stored as long as you are subscribed to
                one of the newsletters. When you end the subscription, it will automatically be deleted.</p>
            <p>If you need to delete your Refactoring PHP account, just contact me at christoph@christoph-rumpel.com. Then we will
                delete all of your data.</p>
            <h2>Security</h2>

            <p>We are committed to ensuring that your information is secure. In order to prevent unauthorised access or
                disclosure, we have put in place suitable physical, electronic and managerial procedures to safeguard
                and
                secure the information we collect online.</p>

            <h2>Controlling Your Personal Information</h2>

            <p>You may choose to restrict the collection or use of your personal information in the following ways:</p>
            <ul>
                <li>Don't subscribe to one of the newsletters.</li>
                <li>Unsubscribe from one of the newsletters. (unsubscribe link in the newsletter)</li>
            </ul>

            <p>We will not sell, distribute or lease your personal information to third parties unless we have your
                permission or are required by law to do so.</p>

            <h2>Website analytics</h2>
            <p>We are using the privacy-focused analytics tool <a href="https://usefathom.com/">fathom</a> which is NOT using any cookies.</p>

            <p>No personal data is being stored, so we only get an overview of how many people visited which site.</p>


            <h2>Our Details</h2>
            <p>Refactoring PHP is owned and operated by Christoph Rumpel.</p>
            <p>You can contact us at: christoph@christoph-rumpel.com</p>

            <p>Our business is located at:<br>

                Christoph Rumpel<br/>
                Wienergasse 39/11<br/>
                2380 Perchtoldsdorf<br/>
                Austria</p>
        </article>

    </main>
</x-main-layout>
